...
export async function DELETE(request, { params }) {
  ...
  try {
    ...
    const result = await db.collection("subcategories").deleteOne({ slug: id });
    
    if (result.deletedCount === 0) {
      return NextResponse.json({ error: "Subcategory not found" }, { status: 404 });
    }
    
    // Also delete any subsubcategories of this subcategory
    await db.collection("subcategories").deleteMany({ parentSlug: id });
    
    // Update products that were linked to this subcategory
    await db.collection("products").updateMany(
      { subcategorySlug: id },
      { $set: { subcategorySlug: "" } }
    );
    
    return NextResponse.json({ message: "Subcategory and associated product links successfully updated." });
  } catch (error) {
    console.error("Database error in DELETE subcategory:", error);
